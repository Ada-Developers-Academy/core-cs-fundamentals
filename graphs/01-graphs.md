...
In the graph below each node represents a city. The edges represent flights between those cities. Notice each edge has a cost associated with it. We can use the graph to traverse different paths along the graph and search for the cheapest flight from City A to City B. 
